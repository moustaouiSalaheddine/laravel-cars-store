...
<div class="wrapper">
  <div class="d-md-flex align-items-md-center">
      <div class="h3">Research popular used cars & SUVs</div>
  ...
  <div class="content py-md-0 py-3">
      <section id="sidebar">
          <div class="py-3">
            <form id="filterForm" role="form" method="POST" action="{{ route('filter') }}" enctype="multipart/form-data">
              @csrf
                <h5 class="font-weight-bold">Brands</h5>
                {{-- <ul class="list-group">
                  <li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center category"> All  <span class="badge badge-primary badge-pill"></span> </li> --}}
                  <div class="form-inline d-flex align-items-center py-1"> <label class="tick">All <input type="radio" name="category_id" value="0"> <span class="check"></span> </label> </div>
                  @foreach ($categories as $category)
                    <div class="form-inline d-flex align-items-center py-1"> <label class="tick">{{ $category->name }} {{ $category->id }} <input type="radio" name="category_id" value="{{ $category->id }}"> <span class="check"></span> </label> </div>
                    {{-- <button type="submit" value="{{ $category->id }}" class=""> --}}
                      {{-- <input id="searchBy{{ $category->id }}" name="category_id" value="{{ $category->id }}" type="checkbox" />
                      <label onclick="$('#filterForm').submit()" for="searchBy{{ $category->id }}"> <li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center category">{{ $category->name }}  <span class="badge badge-primary badge-pill">328</span> </li></label> --}}
                    {{-- </button> --}}
                  @endforeach  
                {{-- </ul> --}}
                {{-- <button type="button" class="btn btn-success">Success</button> --}}
                <div class="d-grid gap-2 mt-4">
                  <button type="submit" class="btn btn-primary btn-block btn-lg">Filter</button>
                </div>
            </form>
          </div>
          {{-- <div class="py-3">
              ...
              <div class="row">
                  ...
                  @foreach ($products as $product)
                    <div class="col-lg-4 col-md-6 col-sm-10 offset-md-0 offset-sm-1">
                        <a href="{{ url('products/'.$product->id) }}" style="text-decoration: none; color: #000;">
                            <div class="card"> <img class="card-img-top" src="{{Storage::url($product->image)}}" style="object-fit: cover;">
                                <div class="card-body">
                                    <h6 class="font-weight-bold pt-1">{{ $product->name }}</h6>
                                    <div class="text-muted description" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">{{ $product->description }}</div>
                                    ...
                                    <div class="d-flex align-items-center justify-content-between pt-3">
                                        <div class="d-flex flex-column">
                                            <div class="h6 font-weight-bold">{{$product->price}} $</div>
                                            {{-- <div class="text-muted rebate">48.56</div> --}}
                                        </div>
                                        <a class="btn btn-primary" href='https://wa.me/212639610866/?text="{{ $product->name }} {{ $product->description }} {{ $product->price }}"' target="_blank" data-action="share/whatsapp/share">Whatsapp</a>
